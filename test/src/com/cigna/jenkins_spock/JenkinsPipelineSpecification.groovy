...
package com.cigna.jenkins_spock
 ...
 * You can access a Spock mock for any pipeline step that would exist by using {@link JenkinsPipelineSpecification#getPipelineMock(String)}.
 * </p>
 * <ol>
 ...
 * <p>
 * {@link JenkinsPipelineSpecification#loadPipelineScriptForTest(java.lang.String)}
 * </p>
 * <p>
 * If you have pipeline scripts (i.e. whole Jenkinsfiles) in <b>src/main/resources</b>,
 * you can load them as {@link Script}s and run them during unit tests with {@link JenkinsPipelineSpecification#loadPipelineScriptForTest(java.lang.String)}.
 ...
 * <p>
 * Pipeline Shared Library Global Variables are just pipeline scripts on the classpath, so they can be unit-tested with {@link JenkinsPipelineSpecification#loadPipelineScriptForTest(java.lang.String)}:
 * </p>
 * <pre><code>
setup:
 ...
 * to be able to call pipeline steps during each Specification test suite (see {@link JenkinsPipelineSpecification#setupSpec()} and {@link JenkinsPipelineSpecification#setup()}),
 ...
 * If you need to instrument additional objects, you can use {@link JenkinsPipelineSpecification#addPipelineMocksToObjects(java.lang.Object)}.
 ...
 * <p>
 * {@link JenkinsPipelineSpecification#explicitlyMockPipelineStep(java.lang.String,java.lang.String)}
 * </p>
 * <p>
 * If for some reason you need to mock a pipeline step that does not come from a detectable plugin, you can use
 * {@link JenkinsPipelineSpecification#explicitlyMockPipelineStep(java.lang.String,java.lang.String)} to add the ability to call that pipeline step and get a Mock object
 ...
 * <p>
 * {@link JenkinsPipelineSpecification#explicitlyMockPipelineVariable(java.lang.String)}
 * </p>
 * <p>
 ...
 * {@link JenkinsPipelineSpecification#explicitlyMockPipelineVariable(java.lang.String)} to create and return a {@link PipelineVariableImpersonator} that will intercept all subsequent interactions with that variable
 ...
 * Jenkins-spock provides an override-able {@link JenkinsPipelineSpecification#makeStaticJenkins()} method to allow a test-suite to define its own Jenkins singleton object to use
 ...
 * </p>
 * <p>
 * {@link JenkinsPipelineSpecification#makeStaticJenkins()} will be called at most once by jenkins-spock; the result will be stored and re-used.
 * Test-suites may call {@link JenkinsPipelineSpecification#getStaticJenkins()} to access the static Jenkins singleton.
...
""",
     ...
     * </ol>
     *
     * Also, the big collections are static, so I think we can set them just once instead of at the beginning of each spec.
     * So, each block that builds up e.g. `PIPELINE_STEPS` first checks if the thing it's going to build is empty,
     * and if not, we skip it. This is a difference from the library version.
     *
     * @see #PIPELINE_STEPS
    ...
    def setupSpec() {
        ...
        if (DEFAULT_TEST_CLASSES.isEmpty()) {

            Set<Class<?>> classes = new LocalProjectPipelineExtensionDetector()
                .getClassesOfTypeInPackage(
                    Object.class,
                    // specifying a package is a cigna-oriented change
                    Optional.of('com.cigna')
                )
            ...
            // - all interfaces - nobody will be able to call them, so they don't need to be instrumented.
            // checking for an interface can cause a NoClassDefFoundError on java 11, so we catch that
            // this is a difference from the library version
            classes = classes.findAll { clazz ->
                try {
                    !clazz.isInterface()
