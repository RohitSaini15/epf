...
class PostgresRuleengineSpec extends JenkinsPipelineSpecification {
    ...
    def """when Postgres Rule Engine is called and the scan is run"""() {
		when:
            def postgresruleengine = new PostgresRuleengine( 
			    config: [
					ruleengineType: 'Postgres',
                    branchPattern: 'stuff',
                    postgres: [
            ...
            postgresruleengine.runApplication()

        then:
            1 * getPipelineMock("echo").call('################ Postgres Ruleengine Started ############################')	
	}
}
